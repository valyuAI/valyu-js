...
async function testSearch(valyu) {
  ...
  try {
    const response = await valyu.search(testQuery, {
      searchType: "all",
      maxNumResults: 10,
    });
    ...
    if (response && response.success === true && !response.error) {
      ...
      console.error(
        "Basic search test FAILED. Response did not indicate success or contained an error.",
      );
  ...
  try {
    const response = await valyu.search(newsQuery, {
      searchType: "news",
      maxNumResults: 5,
    });
    ...
    if (response && response.success === true && !response.error) {
      ...
      console.error(
        "News search test FAILED. Response did not indicate success or contained an error.",
      );
      console.error("Full response:", response);
      return false;
    }
  ...
  try {
    const response = await valyu.search(fastModeQuery, {
      fastMode: true,
      maxNumResults: 5,
    });
    ...
    if (response && response.success === true && !response.error) {
      ...
      if (response.results.length > 0) {
        ...
        console.log(
          `First result relevance_score: ${firstResult.relevance_score !== undefined ? firstResult.relevance_score : "not present (expected in fast mode)"}`,
        );
      }
      console.log("Fast mode search test PASSED!");
    } else {
      console.error(
        "Fast mode search test FAILED. Response did not indicate success or contained an error.",
      );
      console.error("Full response:", response);
      return false;
    }
  ...
  try {
    const response = await valyu.search(urlOnlyQuery, {
      urlOnly: true,
      maxNumResults: 5,
    });
    ...
    if (response && response.success === true && !response.error) {
      ...
      if (response.results.length > 0) {
        ...
        console.log(`First result has url: ${!!firstResult.url}`);
        console.log(
          `First result relevance_score: ${firstResult.relevance_score !== undefined ? firstResult.relevance_score : "not present (expected in url only mode)"}`,
        );
      }
      console.log("URL only mode search test PASSED!");
    } else {
      console.error(
        "URL only mode search test FAILED. Response did not indicate success or contained an error.",
      );
      console.error("Full response:", response);
      return false;
    }
    ...
    console.error(
      "URL only mode search test FAILED due to an exception:",
      error,
    );
    return false;
  }
...
async function testContents(valyu) {
  ...
  try {
    ...
    console.log(
      `URLs processed: ${response2.urls_processed}/${response2.urls_requested}`,
    );
  ...
  const testUrls3 = ["https://www.wikipedia.org", "https://www.github.com"];
  ...
  try {
    ...
    console.log(
      `URLs processed: ${response3.urls_processed}/${response3.urls_requested}`,
    );
  ...
  try {
    ...
    if (r0 && r0.status !== "failed" && r0.summary) {
      console.log(
        ...
        JSON.stringify(r0.summary, null, 2),
      );
    }
  ...
  try {
    ...
    if (
      ...
      console.log(
        "Error handling test PASSED - correctly rejected >10 URLs without async",
      );
    } else {
      console.error(
        "Error handling test FAILED - should reject >10 URLs without async",
      );
      return false;
    }
  } catch (error) {
  ...
  try {
    ...
    if (
      !response7.success &&
      response7.error &&
      response7.error.includes("empty")
    ) {
      console.log(
        "Error handling test PASSED - correctly rejected empty array",
      );
    } else {
...
async function testAnswer(valyu) {
  ...
  try {
    ...
    if (response1.success) {
      ...
      console.log(
        `Number of search results: ${response1.search_results.length}`,
      );
  ...
  try {
    ...
    if (response2.success) {
      console.log(
        `Content includes instructions-style response: ${response2.contents.includes("example") || response2.contents.includes("Example")}`,
      );
  ...
  try {
    ...
    if (response3.success) {
      ...
      if (
        response3.data_type === "structured" &&
        typeof response3.contents === "object"
      ) {
        console.log(
          "Structured data received:",
          JSON.stringify(response3.contents, null, 2),
        );
        console.log("Structured output test PASSED!");
      } else {
        console.log(
          "Warning: Expected structured data but received unstructured",
        );
  ...
  try {
    ...
    if (response4.success) {
      console.log(
        `Number of search results: ${response4.search_results.length}`,
      );
  ...
  try {
    ...
    if (
      !response6.success &&
      response6.error &&
      response6.error.includes("required")
    ) {
      console.log(
        "Error handling test PASSED - correctly rejected empty query",
      );
    } else {
  ...
  try {
    ...
    if (
      !response7.success &&
      response7.error &&
      response7.error.includes("Invalid searchType")
    ) {
      console.log(
        "Error handling test PASSED - correctly rejected invalid search type",
      );
    } else {
      console.error(
        "Error handling test FAILED - should reject invalid search type",
      );
      return false;
    }
  } catch (error) {
